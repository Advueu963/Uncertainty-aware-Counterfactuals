...
DATASET_LOADERS = [
    (
        ...
        {"n_samples": 1000},
        torch.tensor(
            np.array([1, 1]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "l_dataset",
        ...
        {"n_samples": 333},
        torch.tensor(
            np.array([0, 0]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "one_moon",
        ...
        {"n_samples": 1000},
        torch.tensor(
            np.array([-0.1, 1]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "ring_dataset",
        ...
        torch.tensor(
            np.array([0, -1.7]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "bubbles_noisy",
        ...
        {"n_samples": 1000},
        torch.tensor(
            np.array([0.1, 0]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "two_moon",
        ...
        {"n_samples": 1000},
        torch.tensor(
            np.array([0, 0.2]).reshape(-1, 2), dtype=torch.float, requires_grad=True
        ),
    ),
    (
        "infinity_dataset",
...
PROPERTY_LOADERS = [
    ...
    # ("sparse", sparse_loss_function,
    #  {"MAX_STEPS": MAX_STEPS, "aleatoric_uncertainty_function": aleatoric_uncertainty_ensemble, "n_points": 10,
    #   "DESIRED_VALIDITY": DESIRED_VALIDITY, "delta": DELTA})
]
...
plt.savefig("visualize_properties_all_datasets_close_decision_boundary.png")
# plt.show()
