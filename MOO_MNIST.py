...
# parser.add_argument("--desired_class", type=int, default=1)
...
def load_epistemic_model():
    lenet_model = LeNet_MNIST(
        drop_prob=0.5, in_channels=1, image_width=28, image_heigth=28
    )
...
def load_vae(latent_dim):
    ...
    vae.load_state_dict(
        torch.load(
            f"vae_artifacts_{latent_dim}/vae.pth",
            weights_only=True,
            map_location=DEVICE,
        )
    )
...
def load_point_of_interest(model, testloader):
    ...
    plt.axis("off")
    plt.savefig(
        f"moo_mnist/{DESIRED_CLASS}/point_of_interest_{idx}.png", bbox_inches="tight"
    )
    plt.close()
...
def get_costs_mnist(
    epiuc_ensemble_model, X, point_of_interest, desired_class=DESIRED_CLASS
):
    """
    ...
    distances = np.linalg.norm(
        X.reshape(-1, point_of_interest.shape[1]) - point_of_interest, axis=1
    )
    return np.vstack(
...
def get_algorithm_setup(vae, point_of_interest, n_obj, population_size=200):
    ...
    sigma = (
        0.1  # small pertubation of original image: 0.1 better than 0.2 better than 0.3
    )
    ...
    from moo_nsgaii.algorithm import (
        NSGA_II_Counterfactual,
    )

    algorithm = NSGA_II_Counterfactual(
        pop_size=population_size,
        original_point=z_ref.cpu().numpy(),
        sigma_features=np.ones(LATENT_DIM) * sigma,
    )
    # algorithm = NSGA3(
    #     pop_size=population_size,
...
def visualize_pareto_point(vae, pareto_points, objectives, name, latent_dim=16):
    pareto_points = (
        vae.decode(torch.Tensor(pareto_points).to(DEVICE)).cpu().detach().numpy()
    )
...
if __name__ == "__main__":
    ...
    m_f_d_d, m_f_d_s, m_s_d_f, m_s_d_s = load_pymoo_problems(
        epiuc_ensemble,
        vae,
        point_of_interest,
        min_val,
    ...
    for name, problem in {
        ...
        algorithm, termination = get_algorithm_setup(
            vae, point_of_interest, n_obj=problem.n_obj, population_size=400
        )
        points, objectives = get_pareto_front(
            problem, algorithm, termination=termination
        )
        if points is None:
