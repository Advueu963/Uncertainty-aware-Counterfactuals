...
def total_uncertainty_entropy(probs):
    ...
    Returns:
        tu: torch.Tensor, shape (n_instances,)
...
def aleatoric_uncertainty_entropy(probs):
    ...
    Returns:
        au: torch.Tensor, shape (n_instances,)
...
def epistemic_uncertainty_entropy(probs):
    ...
    Returns:
        eu: torch.Tensor, shape (n_instances,)

    """
    _, _, eu = entropy_based_uncertainty_quantification(probs)
    return eu.clip(1e-8, 1 - 1e-8)
...
def distance_func(point_of_interest, cf_point_of_interest, X, categorical_features=[]):
    ...
    if len(categorical_features) > 0:
        ...
        cf_point_of_interest = cf_point_of_interest[:, continuous_features]
...
def instability_metric(
    ...
    # print("----------------------------------")
    # print("POINT OF INTEREST: ", point_of_interest[0])
    ...
    final_distance = distance_cf / (1 + distance_original)
    # print("FINAL DISTANCE: ", final_distance[0], final_distance.shape)
...
def discriminative_power(
    ...
    y_train = np.hstack([class_poi, class_cf])
...
def dissimilarity(point_of_interest, cf_point_of_interest, X, categorical_features=[]):
...
def dissparsity(point_of_interest, cf_point_of_interest):
    ...
    return (point_of_interest != cf_point_of_interest).float().mean(dim=1)


def implausability(cf_point_of_interest, X, categorical_features=[]):
    """
    ...
    # print("IMPLAUSIBILITY DISTANCES SHAPE: ", distances.shape)
    # print("IMPLAUSIBILITY COUNTERFACTUAL: ", cf_point_of_interest[0])
    # print("IMPLAUSIBILITY COUNTERFACTUAL SHAPE: ", cf_point_of_interest.shape)
    # print("IMPLAUSIBILITY MIN DISTANCE: ", torch.min(distances, dim=1).values[0])
    # print("-------------------------------")
...
def evisualize_other_cf_methods(
    point_of_interest,
    points,
    y_labels,
    axes,
...
def visualize_au_eu_tu(
    ...
    y_probs_ensemble = predict_probs(ensemble_model, tensor_points)
    total_uncertainty = total_uncertainty_entropy(y_probs_ensemble)
    aleatoric_uncertainty = aleatoric_uncertainty_entropy(y_probs_ensemble)
    epistemic_uncertainty = epistemic_uncertainty_entropy(y_probs_ensemble)
...
def visualze_path_with_underlying(
    ...
    probs_ensemble = predict_probs(model, tensor_points)
    probs = probs_ensemble.mean(dim=1)
    ...
    au_delta_ball = aleatoric_uncertainty_entropy(probs_ensemble_delta_ball).view(
        n_points, -1
    )
