from .utils import sample_delta_ball
import torch
...
def combined_loss_function(
    ...
    **kwargs,
    ...
    """Combined loss function for counterfactual explanations.
    ...
    # print("PROBS ENSEMBLE DELTA BALL SHAPE: ", probs_ensemble_delta_ball.shape)
    ...
    # Calculate the loss
    # print("LOSS COMPONENTS: ", -(p_weight * target_probs.log2()), (eu_delta_ball.mean(dim=1).log2()), -au_delta_ball.max(dim=1).values.log2())
    loss = (
        -(p_weight * target_probs.log2())
        + (eu_delta_ball.mean(dim=1).exp())
    ...
    # print("DELTA BALL GRAD: ", delta_ball.grad)
