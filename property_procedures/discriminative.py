...
def discriminative_loss_function(
    ...
    **kwargs,
    ...
    """Discriminative (Aleatoric) loss function for counterfactual explanations.
    ...
    tu = aleatoric_uncertainty_function(probs_ensemble) + epistemic_uncertainty_function(probs_ensemble)

    target_probs = probs[0, desired_class]

    loss = p_weight * target_probs.log2()
    if target_probs > 0.51:
        loss = loss - lambda_1 * tu.log2()
    loss = loss.mul(-1)
...
def discriminative_loss_function_2(
    point_to_explain,
    counter_factual,
    model,
    probability_function,
    ...
    """Discriminative (Max_Probability) loss function for counterfactual explanations.
    ...
    probs_ensemble = probability_function(model, counter_factual)
    probs = probs_ensemble.mean(dim=1)
    class_indices = torch.arange(probs.shape[1], device=probs.device)
    _ = aleatoric_uncertainty_function(probs_ensemble)

    target_probs = probs[0, desired_class]

    loss = (
        -target_probs.log2() + (probs[0, class_indices != desired_class].max()).log2()
    )
    loss.backward()
