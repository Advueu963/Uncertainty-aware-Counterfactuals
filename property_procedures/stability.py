...
def stability_loss_function(
    ...
    delta_ball_cf = sample_delta_ball(counter_factual.detach().numpy(), delta, n_points)
    #
    # # Extract loss value
    probs_ensemble_delta_ball = probability_function(model, delta_ball_cf)
    au_delta_ball = aleatoric_uncertainty_function(probs_ensemble_delta_ball)
    # pairwise_distances = (TU_delta_ball.view(-1,1) - TU_delta_ball.view(1,-1)).pow(2).mean()
    # # Calculate the loss

    loss = -(au_delta_ball.std())

    loss.backward()

    grad = (
        delta_ball_cf.grad.mean(dim=0, keepdims=True)
        if delta_ball_cf.grad is not None
        else 0
    )
...
def stable_procedure_tu_start_ball(
    point_to_explain,
    probability_function,
    aleatoric_uncertainty_function,
    epistemic_uncertainty_function,
    ...
    while T < MAX_STEPS:
        ...
        # Calculate the loss
        lam = 0.1
        loss = -target_probs_delta_ball.mean() + lam * (
            (tu_delta_ball.mean() - tu_delta_ball_start.mean()) ** 2
            + (tu_delta_ball.std() - tu_delta_ball_start.std()) ** 2
        )
