...
N_EPOCHS = 50
N_ITERATIONS = 5
OPTIMIZATION_METHOD = "adam"  # "adam" or "sgd"
base_ensemble = [
    MLP_Classifier(
        input_shape=2,
        n_classes=2,
...
def visualize_other_cf_methods(point_of_interest, points, y_labels, axes, dataset_name):
    ...
    if counterfactuals_growing_sphere is not None:
        l2_distance_growing_sphere = implausability(
            torch.from_numpy(counterfactuals_growing_sphere.values[0]), points
        )
    else:
    ...
    if counterfactuals_clue is not None:
        l2_distance_clue = implausability(
            torch.from_numpy(counterfactuals_clue.values[0]), points
        )
    else:
    ...
    if counterfactuals_dice is not None:
        l2_distance_dice = implausability(
            torch.from_numpy(counterfactuals_dice.values[0]), points
        )
    else:
    ...
    if counterfactuals_face is not None:
        l2_distance_face = implausability(
            torch.from_numpy(counterfactuals_face.values[0]), points
        )
    else:
...
for i, (dataset_name, loader, kwargs, point_of_interest) in enumerate(DATASET_LOADERS):
    ...
    for j, (property_name, property_function) in enumerate(PROPERTY_LOADERS):
        ...
        l2_distance = [
            implausability(cf_poi[i], points).detach().numpy()
...
l2_stability_data.to_csv(f"plausable_{OPTIMIZATION_METHOD}.csv")
# plt.show()
