...
def save_cfs_tabular(points, y_labels, 
    ...
    np.save(
        os.path.join(
            SAVE_FOLDER,
            f"CFs_{MODEL_NAME}_{dataset_name}_{DESIRED_VALIDITY}_{DELTA}_{OPTIMIZER_LR}_{LAMBDA_1}_{LAMBDA_2}_{PROB_WEIGHT}_{N_POINTS}_test.npy",
        ),
        res,
        allow_pickle=True,
    )
...
if __name__ == "__main__":
    ...
    for dataset_name in data_files:
        ...
        save_cfs_tabular(X_test, y_test,
                         X_points, X_points_close,
                         dataset_name,
                         categorical_features_lists=categorical_features_lists,
                         immutable_features_lists=immutable_features_lists,
                         configs=configs
