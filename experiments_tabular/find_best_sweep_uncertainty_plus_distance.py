    ...
    data = np.load(
        os.path.join(
            os.path.join(DATA_FOLDER,
                         "property_tabular"),
            "CFs_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}_uncertainty+distance_test.npy".format(model_name, dataset_name, *sweep_kwargs)
        ),
        allow_pickle=True,
    ).item()
...
def get_point_of_interest_and_closest(X_test, y_test, num_points=100, rng=None):
    if rng is None:
...
def get_dataset_sweep(sweep_kwargs):
    ...
    for dataset_name in data_files:
        ...
        rng = np.random.default_rng(42)
        X_points, X_points_close, y_points, y_points_close = get_point_of_interest_and_closest(X_test, y_test, rng)
        ...
        for property_name in PROPERTY_NAMES:
            saved_cfs = load_property_data(MODEL_NAME, dataset_name, property_name, sweep_kwargs)
            ...
            row = {
                ...
                "method": property_name + "_uncertainty_distance",
...
if __name__ == "__main__":
    ...
    for sweep_kwargs in lines:
        ...
        mask = score_data["method"].isin(["combined_uncertainty_distance"])
        ...
        # print("FINAL DATA SCORE: ", data_score)
    ...
    best_data.to_csv(f"best_sweep_results_{MODEL_NAME}_uncertainty_plus_distance_test.csv", index=False)
