...
if __name__ == "__main__":
    ...
    for dataset_name in data_files:
        ...
        print(
            f"Starting experiment on dataset: {dataset_name} with method {method_name}"
        )
        ...
        df_points = original_df.iloc[
            data["idx_test"][y_test != DESIRED_CLASS][points_of_interest]
        ].astype(np.float32)
        df_points_closest = original_df.iloc[
            data["idx_test"][y_test != DESIRED_CLASS][points_closest_to_interest]
        ].astype(np.float32)
        ...
        match method_name:
            ...
            case "CLUE":
                ...
                print(
                    "Chosen hyperparameter index for CLUE: ", choosen_hyperparam_index
                )
            ...
            case "FACE":
                choosen_hyperparam_index = int(os.getenv("SLURM_ARRAY_TASK_ID", 0))
                print(
                    "Chosen hyperparameter index for FACE: ", choosen_hyperparam_index
                )
                HYPERPARAMS = hyper_params_method[choosen_hyperparam_index]
                #
        ...
        if method_name == "FACE":
            ...
            cfs_closest, cfs_times_closest = method.get_counterfactuals(
                df_points_closest_unique
            )
            for idx in df_points_closest_unique.index:
                for _ in range(
                    1, len(df_points_closest.index[df_points_closest.index == idx])
                ):
                    cfs_closest = pd.concat(
                        [
                            cfs_closest,
                            cfs_closest.iloc[
                                [df_points_closest_unique.index.get_loc(idx)]
                            ],
                        ],
                        ignore_index=True,
                    )
                    cfs_times_closest.append(
                        cfs_times_closest[df_points_closest_unique.index.get_loc(idx)]
                    )
        else:
            cfs_closest, cfs_times_closest = method.get_counterfactuals(
                df_points_closest
    ...
    print(
        "Saving results to: ",
        os.path.join(SAVE_FOLDER, f"CFs_{MODEL_NAME}_{method_name}_all_datasets.csv"),
    )
    erg.to_csv(
        os.path.join(SAVE_FOLDER, f"CFs_{MODEL_NAME}_{method_name}_all_datasets.csv"),
        index=False,
